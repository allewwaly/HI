...
void cr3_callback(vmi_instance_t vmi, vmi_event_t *event) {
	...
	if (page->vaddr >= 0xffff808000000000 && page->vaddr <= 0xffff87ffffffffff){
		printf("page dtb is:0x%lx\tvaddr is:0x%lx\tpaddr is:0x%lx\tsize is:0x%x\n",
		event->reg_event.value, page->vaddr, page->paddr, page->size);
	}
        loop=loop->next;
    }
    free_va_pages();
