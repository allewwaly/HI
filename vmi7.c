//specify hypercall, move reg read inside the if clause, add hvm specific hypercalls
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <inttypes.h>
...
void mm_callback(vmi_instance_t vmi, vmi_event_t *event) {
	...
	event->mem_event.hypercall,rax,rbx,rcx,rdx,rsi,rdi);
        ...
        printf("\tEvent on same page: %lx, but not the hypercall: %s",event->mem_event.gla,event->mem_event.hypercall);
        vmi_clear_event(vmi, event);
